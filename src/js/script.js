function onGoogleLoaded() {
  ...
  function performSearch(id) {
    ...
    SpotifyApi.sendRequest(
      SPOTIFY_API + "/albums/" + id,
      function (error, album) {
        ...
        getAlbums(uri, album.name, albumsCallback, {
          markets: markets,
          ids: ids,
          count: count,
        });
    ...
    SpotifyApi.sendRequest(uri, function (error, albums) {
      albums.items.forEach(function (el) {
        ...
        var condition = Config.getExactSearch()
          ? el.name == alName
          : elName.indexOf(alName) == 0 || alName.indexOf(elName) == 0;
  ...
  function getAlbumInfo(name, albums, id) {
    ...
    albums.forEach(function (album) {
      ...
      var copyrights = d3.map(album.copyrights, function (d) {
        return d.type;
      });
      ...
      html +=
        ...
        " tracks on " +
        album.tracks.items.pop().disc_number +
        " discs";
      html += "</a></li>";
      ...
      html +=
        ...
        "</p>";
      html +=
        '<p class="uri"><a href="https://api.spotify.com/v1/albums/' +
        album.id +
        '">' +
        album.uri +
        "</a></p>";
      if (copyrights.get("C")) {
        html += '<p class="copyrights">&copy; ' + copyrights.get("C").text;
        +"</p>";
      }
      if (copyrights.get("P")) {
        html +=
          '<p class="copyrights">&#8471; ' + copyrights.get("P").text + "</p>";
      }
      div.innerHTML = html;
        ...
        function (error, albums) {
          ...
          nest.forEach(function (el) {
            infoContainer.insertBefore(
              getAlbumInfo(el.key, el.values, album.id),
              infoContainer.firstChild
            );
          });
    ...
    marketsList.forEach(function (markets, i) {
      markets.forEach(function (market) {
        ...
        var el = [
          name,
          result.count[market],
          result.count[market] + " versions available",
        ];
        el.spid = ids[i];
        data.push(el);
      });
